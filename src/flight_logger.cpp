...
{
    if (!status.ok())
        throw std::runtime_error("MCAP writer error: " + status.message);
}
...
{
public:
    explicit Impl(FlightRecorderOptions options)
    ...
    uint32_t allocate_channel_id()
    {
        ...
        if (this->running_)
            throw std::logic_error("cannot register channel after recorder start");
        ...
        std::string reason)
    {
...

        {
            ...
            if (this->stopping_)
                throw std::logic_error("cannot trigger a stopping recorder");

            for (auto& channel : this->channels_)
                channel->request_freeze();

            this->requests_.push_back(TriggerRequest{
                trigger_time_ns,
...
private:
    ...
    void handle_trigger(const TriggerRequest& request)
    {
        const uint64_t begin_time =
            request.trigger_time_ns > this->options_.pre_trigger_ns
                ? request.trigger_time_ns - this->options_.pre_trigger_ns
                : 0;
        const uint64_t end_time =
            saturated_add(request.trigger_time_ns, this->options_.post_trigger_ns);
        ...
        std::this_thread::sleep_for(
            std::chrono::nanoseconds(this->options_.post_trigger_ns));

        const uint64_t post_begin =
            request.trigger_time_ns == std::numeric_limits<uint64_t>::max()
                ? request.trigger_time_ns
                : request.trigger_time_ns + 1;
        ...
        if (post_begin <= end_time)
        {
            ...
            this->acquire_all(post_begin, end_time);
            try
            {
                this->write_acquired_records(writer, channel_ids);
            }
    ...
    std::unordered_map<uint32_t, mcap::ChannelId>
    register_mcap_channels(mcap::McapWriter& writer)
    {
        ...
        for (const auto& flight_channel : this->channels_)
        {
            ...
            metadata["ring_capacity"] =
                std::to_string(flight_channel->ring_capacity());
    ...
    void acquire_all(uint64_t begin_time, uint64_t end_time)
    {
        ...
        while (acquired_count < this->channels_.size())
        {
            for (std::size_t i = 0; i < this->channels_.size(); ++i)
            {
                ...
                if (this->channels_[i]->acquire_frozen(begin_time, end_time))
                {
                    acquired[i] = true;
                    ++acquired_count;
                }
            ...
            if (acquired_count < this->channels_.size())
                std::this_thread::sleep_for(std::chrono::milliseconds(1));
        }
    }

    void write_acquired_records(
        mcap::McapWriter& writer,
        const std::unordered_map<uint32_t, mcap::ChannelId>& channel_ids)
    {
        for (;;)
        {
            ...
            for (auto& channel : this->channels_)
            {
                ...
                if (!next ||
                    channel->current_timestamp() < next->current_timestamp())
                {
                    next = channel.get();
                }
            }
...
void FlightRecorder::register_channel_erased(
    std::unique_ptr<IFlightChannel> channel)
{
